...
export default function DemoLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className={`${fraunces.variable} ${inter.variable} demo-root min-h-screen bg-[#0A0A0A] text-[#F3EEE3]`}>
      <header className="sticky top-0 z-40 border-b border-bone/5 bg-[#0A0A0A]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <Link
            href="/book"
            className="rounded-full border border-bone/15 px-4 py-2 text-sm font-medium text-[#F3EEE3]/90 transition hover:border-bone/30 hover:bg-bone/5"
          >
            Book a call
          </Link>
        </div>
