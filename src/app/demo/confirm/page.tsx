  ...
  if (loading) {
    return (
      <div className="flex min-h-[calc(100vh-72px)] items-center justify-center">
        <div className="text-sm text-bone/50">Looking up your restaurant…</div>
      </div>
    );
  }
  ...
  if (error || !place) {
    return (
      <div className="mx-auto max-w-xl px-6 py-20 text-center">
        ...
        <p className="mt-3 text-bone/60">{error || 'Try searching again.'}</p>
    ...
    <section className="relative overflow-x-hidden">
      ...
      <div className="mx-auto max-w-5xl px-6 py-16">
        <button
          ...
          className="mb-8 inline-flex items-center gap-2 text-sm text-bone/60 transition hover:text-bone"
        >
            ...
            {place.photoUrl ? (
              <div className="relative aspect-[16/10]">
                ...
                <div className="absolute bottom-5 left-5 right-5">
                  <div className="inline-flex items-center gap-2 rounded-full bg-obsidian/50 px-3 py-1 text-xs text-bone/80 backdrop-blur">
              ...
              <div className="flex aspect-[16/10] items-center justify-center bg-bone/[0.03] text-bone/30">
                No photo available
              </div>
            )}
            <div className="p-6">
              <h2 className="display text-3xl leading-tight">{place.name}</h2>
              <p className="mt-1 text-sm text-bone/60">{place.address}</p>
                ...
                {place.rating ? (
                  <span className="chip">
                    ...
                    {place.userRatingCount ? (
                      <span className="text-bone/40">({place.userRatingCount})</span>
                    ) : null}
                  </span>
                ) : null}
          ...
          <div className="flex flex-col justify-between">
            <div>
              ...
              <p className="mt-4 text-bone/60">
                We&apos;ll build a one-time AI host for <span className="text-bone">{place.name}</span> using its
